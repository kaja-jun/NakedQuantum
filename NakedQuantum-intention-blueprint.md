...
- **Background passes** — work runs between sessions and on events; rest is scheduled function, not absence of the system.
- **State** — attending, wrong, silent, obsessed — expressed in tiers, directives, and ledger rows, not a single on/off lamp.
- **One body** — creation, witness, decay, and void live in the same Abyss world (not four competing “realms” that contradict the metaphor).
...
### 4.1 Abyss — you and the witness

Everything that belongs to **your writing** and the **witness loop’s attention** to that writing lives here — including topology, decay, and process.
...
| **Process** | Witness thinking made readable: conclusions, passes, calibration, returns, silences — **text first**. |
...
### 5.1 Interaction spectrum (not a chatbot — not a dead log)

We refuse two bad poles:

| Pole | Why we reject it |
|------|------------------|
| **Chat-primary** | Relationship thread, performative empathy, “assistant” identity. |
| **Log-only** | User never engages; Guardian becomes syslog with aura. |

**Intention:** **Process-first, engageable on demand.**
...
**Slip guardrails:** No open-ended “how are you feeling” default. No Sanctuary bleed. Strip and summon stay **witness-shaped** (theory line, ledger reckoning, directive) even when the UI allows a reply bubble.

Language interface remains **optional and costly** (BYOK OpenRouter on PWA; local Ollama on desktop) — not the default proof of life.
...
```text
you write
  → Track A: Cartographer fast map (always, phone-safe)
  → Track B: sentence-level geometry (desktop-first matrix)
  → Synapse: fused snapshot (math-first payload to worker / local pass)
  → Local pass: rules + SILENCE without LLM on every save
  → Guardian (rare): theory + ledger + optional directive
  → Field + ledger update → you notice → you write about that
```
...
**Resonance / Omniscient Cartographer** (from design discussions) are **names for the fused synapse + matrix story**, not a second app. One vault, one witness, richer substrate.

3D Sky remains **weather on the same field** — not a separate epistemology.
...
- Sentence matrix, background passes, Wave 2/3 — **thinkable here first**.
- Layer 4 (engine models the *shape* of its characteristic errors for this mind) — desktop + history + local model, not PWA fantasy.
...
| Keeps (first-class) | Honest degrade |
|-------------------|----------------|
| Write, vault, sync, Cartographer, Watcher | Heavy 3D (simplified sky or defer) |
| Process log, field mutation, mesh, ledger | Sentence matrix at full scale |
| Tiered summon, strip, OpenRouter BYOK | Ollama, background between sessions |
| Pulse passes on open / save | Wave 2/3 analyst/mirror in-tab |

Kaja writes mostly on phone; desktop arrives for **depth**, not because phone is “lite NakedQuantum.”
